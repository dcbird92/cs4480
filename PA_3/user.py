...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def arp_reply_virtual(self, dst_mac, dst_ip):
        src_ip = self.servers[self.index]
        self.index_change()
    ...
    def _packet_in_handler(self, ev):
        ...
        if eth.ethertype == ether.ETH_TYPE_ARP:
            ...
            if arp_msg.opcode == arp.ARP_REQUEST:
                if not self.ip_mac.contains(arp_msg.dst_ip):
                    out_pkt = self.arp_reply_virtual((arp_msg.src_mac, arp_msg.src_ip))

                    actions = [ofp_parser.OFPActionOutput(in_port)]
                    out = ofp_parser.OFPPacketOut(datapath=datapath, in_port=ofp.OFPP_ANY,
                                                  data=out_pkt.data,actions=actions,
                                                  buffer_id=OFP_NO_BUFFER)
                    datapath.send_msg(out)
            return

        elif eth.ethertype != ether.ETH_TYPE_ICMP:
            return
